...
export function TechElements({ scrollData, introAnimationProgress }: ComponentProps): JSX.Element {
  ...
  useFrame(({ clock }) => {
    ...
    groupRef.current.children.forEach((element, i) => {
      ...
      if (introAnimationProgress < 1) {
        ...
        const [x, y, z] = interpolatePosition(el.initialPosition, el.targetPosition, easing);
    ...
    <group ref={groupRef}>
      {elements.map((element, i) => (
        ...
        >
          ...
          {element.shapeType === 2 && <torusGeometry args={[0.5, 0.2, 16, 32]} />}
